-- ============================================================================
-- 06_customer_segmentation_rfm.sql
--
...
-- This is the most involved file in Phase 2, so it's saved for last.
--
...
-- ----------------------------------------------------------------------------
-- CTE 1: reference_date
-- This is historical data with no real "today," so we treat the single most
-- recent purchase in the whole dataset as our snapshot date -- "recency" is
-- measured relative to that, not to the actual current calendar date.
-- ----------------------------------------------------------------------------
...
-- One row per (person, order), same NOT IN status filter used for revenue
-- in earlier files -- a canceled order isn't real purchasing behavior.
-- ----------------------------------------------------------------------------
customer_orders AS (
    SELECT
...
-- ----------------------------------------------------------------------------
-- CTE 3: order_totals
-- Revenue lives on order_items, one row per line item, so this collapses
-- each order down to a single spend total before it's attached to a person.
-- ----------------------------------------------------------------------------
order_totals AS (
...
-- ----------------------------------------------------------------------------
-- CTE 4: rfm_base
-- One row per real person: their most recent purchase date (feeds Recency),
-- how many distinct orders they placed (Frequency), and total spend
-- (Monetary) -- the raw ingredients, not yet scored into quintiles.
-- ----------------------------------------------------------------------------
rfm_base AS (
    SELECT
...
-- customer row -- normally a CROSS JOIN (no ON condition, full cartesian
-- product) is a red flag, but it's safe here specifically because
-- reference_date has exactly one row, so it can't multiply rfm_base's rows.
--
-- Scoring direction, one column at a time:
--   - r_score: sorted by last_order_date ASCENDING, so the customer who
--     bought longest ago (worst recency) lands in bucket 1, and the most
--     recent buyer (best recency) lands in bucket 5.
--   - f_score / m_score: sorted ASCENDING on frequency / monetary directly,
--     so the lowest spender/least-frequent buyer is bucket 1 and the
--     highest is bucket 5. Higher is better for both, so this is the
--     natural direction (no need to flip it like recency).
...
-- customers placed exactly one order, so `frequency` has a massive block of
-- tied values at 1. NTILE has no way to split a tied block "correctly" --
-- with no secondary sort key, Postgres would place those tied rows into
-- buckets 1 through 5 in whatever order it happens to encounter them,
-- which is arbitrary and not reproducible from run to run. Adding
-- `, rb.monetary ASC` as a second ORDER BY key doesn't fix the underlying
-- fact that frequency barely distinguishes one-time buyers from each other
-- (that's a real property of this dataset, not a bug) -- it just makes
-- WHICH bucket a given tied customer lands in deterministic and explainable
-- (lowest-spending one-time buyers sort toward bucket 1, highest-spending
-- toward bucket 5) instead of effectively random.
-- ----------------------------------------------------------------------------
rfm_scores AS (
    SELECT
...
-- ----------------------------------------------------------------------------
-- Query 1: per-customer RFM detail, top 50 by combined score
--
...
--
-- This LIMIT 50 is only here to keep the result reviewable in the SQL
-- editor -- drop it when this query is actually used to feed the JSON
-- export in Phase 3, since the dashboard needs every customer, not just 50.
-- ----------------------------------------------------------------------------
SELECT
    customer_unique_id,
    recency_days,
...
-- repeated here rather than reused from Query 1 above -- same reasoning as
-- file 5 repeating its order_delay CTE across two queries.
--
-- This is the sanity-check query: it rolls the per-customer detail up into
-- one row per segment, so you can eyeball whether "champion" customers
-- really do have the best average recency/frequency/monetary numbers (they
-- should, since that's how the segment was defined), and roughly how many
-- customers fall into each bucket.
-- ----------------------------------------------------------------------------
WITH reference_date AS (
    SELECT MAX(order_purchase_timestamp) AS snapshot_date
    FROM orders
